...
export class CallStore {
  ...
  startCall: MakeCallFn = async (number: string, ...args) => {
    // Demo mode: start mock call instead of real SIP call
    if (DEMO_MODE) {
      console.log('[startCall] Demo mode - starting mock call to:', number)
      const success = await demoStore.startMockCall(number)
      if (success) {
